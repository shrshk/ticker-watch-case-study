...
| **H4** | `${VENDOR_API_KEY:?…}` was required even for `PRICE_SOURCE=simulated`. A reviewer without a key could not run the stack at all. | **fixed** — optional; `VendorSource` fails loudly when the key is actually needed |
...
- **Centrifugo's hot-channel knee is between 25k and 50k on this hardware** (§8.10):
  clean at 25k, superlinear by 50k (~40ms steady state), unmeasurable at 100k
  because the VM saturated - reproduced end to end. Three nodes on one machine
  split CPU three ways and did not lower per-broadcast time or client latency
  (§8.11); the more-nodes lever's payoff is unmeasured and needs separate hosts. Attributing the 100k result needs a second host for the
